...
|---------------|---------|--------------------------------|
| ユニットテスト （単体テスト）| ソフトウエアの最小単位に行うテスト | 随時（開発途中）、Push時などが多い  |
| 統合テスト （結合テスト）   | ユニットテストより高水準のテストを指し、通常複数のアプリケーションやDB相互のやり取りなどの境界に注目し、大きな塊のコードのテスを行う。      | アプリケーションが肥大化し、外部システムと統合した状態になった時 |
...
本資料では主として基本部分にあたる単体テスト、及び統合テストについて実践例を示します。

## テスト戦略
* テストの考え方として、幾つかの可能性があります。実行する際、特に問題になりやすいのがテストをいつ、何を、どこまで網羅的に実行するか？、の3つです。

### テストの実行タイミング
　テストの実行タイミングはプロジェクトの状況によって変化しますが、基本的には、機能や求められる要件・仕様が固まり、今後も継続的なアップデートが見込まれる時期には実行するべきです。
これは、テストの性質上、テストしたい主要機能以外の変更がリグレッションを引き起こすか？、に焦点を当てるため、予期せぬバグの混入を検知できるようにするためです。
逆に実験や必ずしも主要機能が定まっていない段階からテストを行うことをテスト駆動開発、と呼びますが本資料ではそちらは対象外とし、上記条件を満たすタイミングでのテストを想定します。

### テストの内容
　テストの内容に関して、何をどこまで実行するか？、について記載します。まず何をテストするべきか？、の考え方は基本的に「対象となる機能に関連するものは全てテスト対象」となります。  
ですが、全てを網羅的に実行するのは現実的ではないので、本資料では、「ユーザーの見える機能や遭遇する事象に関して」テストする立場をとります。ただし、Streamlit の場合、User Interface とコードの変更が1対1で紐付くため、UIとしての挙動のテストよりも、計算処理やエラー処理などの機能面で想定した動作をすることを保証するためにテストを実施します。
...
* pytest におけるテストの構造としては、主要な要素を挙げると、

1. テスト実行の前処理・後処理に相当するfixture
2. テスト対象の関数・クラス等に入力するパラメータ
3. テスト対象の振る舞いを決めるassert

の3つで構成されています。本プロジェクトの場合、ファイルを跨って共通化する前処理1をconftest.pyに記述し、テストの実行ファイルであるtest_kalman.pyにテストパラメータとassertを組み込んでいます。
ここではサンプルコードを元に、個々の振る舞いを記述します。
